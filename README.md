...
### 2. 文件夹说明
*   **`results/`** (运行结果)
    *   每次运行都会在这里生成一个新的文件夹（带时间戳），里面包含 CSV 数据表和运行日志。
*   **`reports/`** (分析报告)
    *   存放生成的精美 Markdown 报告，如《市场分析报告》、《谈判记录》等。
*   **`docs/`** (文档资料)
    *   存放项目手册、安装指南等说明文件。
*   **`_archive/`** (归档箱)
    *   存放旧的测试代码、废弃的脚本和旧的可视化看板。如果您觉得根目录太乱，不想用的东西都在这里。
...
1.  查看历史存档 ID：
    ...
    # e4f5g6h 初始化项目      <-- 上一个版本
    ```
2.  回退（保留代码修改，只是撤销存档动作）：
    ```bash
    git reset --soft HEAD~1
    ```
3.  **强力回退**（完全丢弃最新的修改，回到上一个版本，**危险操作**）：
    ```bash
